...
若检查发现目标服务未启动，先按当前环境约定命令自动拉起；仅在自动拉起失败后，才停止并报告用户。

### 7.6 环境自检与自动拉起规则

当执行包明确依赖本地运行环境时，开始读业务代码前先做运行环境自检：

1. 先确认本次任务应使用的环境口径（开发环境 / dev:ai / dev:cc）。
2. 先检查目标端口是否已监听，再校验对应健康接口是否可用。
3. 若目标服务未启动或健康检查失败：
   - 直接按该环境的约定启动命令自动拉起对应服务；
   - 启动成功后继续执行任务，不需要因此中断；
   - 启动失败时，才按“环境阻塞”格式上报。
4. 不把 `.env` / `.env.ai` / `.env.cc` 文件存在视为服务可用的证据。
5. 环境恢复动作不算业务改动，不需要为此单独等待确认，除非用户明确禁止自动启动。

默认启动命令：
- 开发环境：各端目录下执行 `npm run dev`
- AI 测试环境：各端目录下执行 `npm run dev:ai`
- Claude Code 环境：各端目录下执行 `npm run dev:cc`
