...
SmartDine V1.1 已从 P1 收口进入 P2 深化收敛阶段。P1 已完成 FAQ + API + Admin + H5 最小闭环,P2 当前目标是提升问答可信度、Admin 可运营性、工程洁净度。

-   P0：已完成
-   P1：已完成，已完成总体验收
-   P2：已启动 / 准备进入第一轮执行
-   P3：未启动，后续考虑 Admin + H5 视觉与交互收敛
...
-   检索低置信度与复合问题兜底
-   问答可观测性最小化
-   AI 模型与 Prompt 配置后台化方案设计

下一步：

1. 先完成 P2 启动基线确认
2. 再处理工程清理
3. 再进入推荐、检索、可观测性和 AI 配置方案
4. Admin 认证、AI 配置最小落地、bundle 拆包、Query Rewrite 验证后置
...
| 端    | 开发环境 | AI 测试环境 | Claude Code 环境 |
| ----- | -------: | ----------: | ---------------: |
| API   |     3000 |        3300 |             3301 |
| Admin |     5174 |        5274 |             5275 |
| H5    |     5173 |        5273 |             5276 |

> Claude Code 专用环境使用 `dev:cc` 命令，读取各端 `.env.cc` 文件。三套环境端口完全隔离，可同时运行不冲突。Codex / 其他 AI 使用 `dev:ai`，Claude Code 使用 `dev:cc`。
