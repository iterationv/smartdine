...
const buildFaqPolishMessages = (input) => {
    if (!input.matchedFaq) {
    ...
    return [
        {
            role: 'system',
            content: '你是餐饮场景的 AI 客服助手。现在已经有一条标准 FAQ 答案，你的任务是把它说得更自然、更口语化。不要改写事实，不要补充 FAQ 里没有的信息。请用中文回答，控制在 1 到 3 句话，不要使用技术术语，也不要说“根据 FAQ”之类的系统措辞。',
        },
        {
            role: 'user',
            content: [
...
const buildFallbackMessages = (input) => {
    return [
        {
            role: 'system',
            content: '你是餐饮场景的 AI 客服助手。当前没有命中的 FAQ，你只能回答餐厅服务相关、用户帮助相关的内容。不要编造未知信息，应优先给出保守、安全、可执行的引导。你可以建议用户查看首页、健康页面、记录页面，或联系人工客服。请用中文回答，控制在 1 到 3 句话，不要假装知道具体入口名称、页面细节或未确认的规则。',
        },
        {
            role: 'user',
...
export const askLLM = async (input) => {
