...
import { addFaq, deleteFaq, getFaqList, getMatchedFaq, updateFaq, } from './faq.js';
...
const parseQuestionBody = async (request) => {
    let body;
    try {
...
const parseCreateFaqBody = async (request) => {
    let body;
    try {
        body = await request.json();
    }
...
const parseUpdateFaqBody = async (request) => {
    let body;
    try {
        body = await request.json();
    }
...
const jsonUtf8 = (c, data, status = 200) => {
    c.header('Content-Type', 'application/json; charset=utf-8');
    c.header('X-Content-Type-Options', 'nosniff');
    return c.body(JSON.stringify(data), status);
};
const isFaqBusinessError = (error) => {
